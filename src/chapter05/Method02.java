...
/*
    ...
    1) 반환타입(return type)
    - 메서드의 자겁 수행 결과인 반환값의 타입을 명시
    - void : 반한값이 없는 경우

    2) 매개변수(paramether) 선언
    - 메서드가 작업을 수행하는데 필요한 값들을 제공받기 위한 것
    - 필요한 값의 개수만큼 변수를 선언, 데이터 타입과 변수명을 심포(,)로 구분하여 나열
    cf) 변수들의 데이터 타입이 같더라도 타입 생략 X

    ex) int add(int x, y) {} - errot

    
    2. 메서드의 구현부
    : 메서드 선언부 다음에 오는 중괄호 {}
    : 반환타입이 void가 아닐 경우 return문을 반드시 포한!
...
public class Method02 {
    public static void main(String[] args) {
        ...
        // example.add(1);
        // example.add(10, 20, 30);     - Error (인자값은 매개변수의 수를 넘을 수 없다)
