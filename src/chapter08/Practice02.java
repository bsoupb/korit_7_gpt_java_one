...
public class Practice02 {
    public static void main(String[] args) {
        ...
        Fruit[] fruits = {apple, orange, banana, mango};    // mango 자동 형 변환
        ...
        // 고양이 고양이로바꿈 = (고양이) 애니멀;  - error
        // : 형 변환을 시킬 수 없는 상황

        if(mango instanceof Fruit) {
            Fruit mangoAsFruit = (Fruit) mango;     // 과일의 색상은 golden이며, 모양은 타원형입니다.
            mangoAsFruit.describe();
        }

        if(banana instanceof TropicalFruit) {
            TropicalFruit bananaATropical = (TropicalFruit) banana;
            bananaATropical.tropicalFeature();      // 해당 내용 존재 X
        } else {
